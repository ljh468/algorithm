...
def solution(users, n):
    ...
    tiers = []  # users의 유저를 담을 리스트
    tiers.append([users[0]])  # users의 첫번째 유저를 추가 (이중배열로 구성)
    ...
    for user in users:  # 새로운 유저를 추가
        ...
        print('tears 확인 : ', tiers)
        print('cnt 확인 : ', cnt)
        for c in range(0, len(tiers)):  # 매칭될 집합이 있는지 탐색
            # 매칭 집합의 +-허용티어 점수에 포함되면
            if appand == 0 and user <= int(tiers[c][0]) + int(cnt[c]) and user >= int(tiers[c][0]) - int(cnt[c]):
                tiers[c].append(user)  # 매칭 집합에 유저를 추가
                if len(tiers[c]) == n:  # 집합안 유저의 수가 N명이 되면
                    matchCnt += 1  # 매칭 수 1추가
                    tiers.pop(c)  # 매칭 집합 제거
        ...
        if appand == 0:  # 새로운 유저가 매칭이 되지않았다면
            tiers.append([user])  # 새롭게 생성되는 집합은 첫번째 유저의 티어 점수 +-2까지의 유저들만 허용하는 집합으로 생성
