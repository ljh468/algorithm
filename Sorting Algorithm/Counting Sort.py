# 계수 정렬
# 특정한 조건이 부합할 때만 사용할 수 있지만 매우 빠르게 동작하는 정렬 알고르즘입니다.
...
# 계수 정렬의 사간 복잡도와 곡간 복잡도는 모두 O(N+K)
# 계수 정렬은 때에 따라서 심각한 비효율성을 초래할 수 있슴
# 데이터가 0과 999,999로 단2개만 존재하는 경우
# 계수 정렬은 동일한 값을 가지는 데이터가 여러 개 존재할때 효과적으로 사용할 수 있어요
