...
public class _FruitTree {
  ...
  public static void main(String[] args) {
    ...
    // 1. 1번 나무에서 시작
    // 2. 2번 나무에서 시작
    int result = Math.max(go(0, 0, m),     // 1번 나무에서 시작
  ...
  static int go(int idx, int tree, int cnt) {
    if (cnt < 0) {
      return Integer.MIN_VALUE; // 이동 가능 횟수를 초과하면 불가능한 상태
    }
    if (idx == n) {
      return 0;                // 모든 시간이 지나면 종료
    }
    ...
    int maxFruit = Math.max(go(idx + 1, tree ^ 1, cnt - 1), // 다른 나무로 이동 (XOR 연산 활용)
